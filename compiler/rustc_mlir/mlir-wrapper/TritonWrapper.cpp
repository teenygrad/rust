...
#define DEFINE_SIMPLE_CONVERSION_FUNCTIONS(TYPE, REF)                          \
  static inline TYPE *unwrap(REF ref) {                                        \
...
static Type *allocType(Type type) { return new Type(type); }

static Attribute *allocAttribute(Attribute attr) { return new Attribute(attr); }
...
MLIRRustResult
tritonRustRegisterTritonDialect(MLIRDialectRegistryRef registry) {
  unwrap(registry)->insert<mlir::triton::TritonDialect>();
  return MLIRRustResult_Success;
}

MLIRRustResult
tritonRustRegisterTritonGPUDialect(MLIRDialectRegistryRef registry) {
  unwrap(registry)->insert<mlir::triton::gpu::TritonGPUDialect>();
  return MLIRRustResult_Success;
}

int tritonRustIsAvailable(void) { return 1; }

//===----------------------------------------------------------------------===//
// Triton Pointer Type
//===----------------------------------------------------------------------===//

MLIRTypeRef tritonRustPointerTypeGet(MLIRTypeRef pointeeType,
                                     int addressSpace) {
...
MLIRAttributeRef tritonRustBlockedEncodingAttrGet(
    MLIRContextRef ctx, size_t rank, const unsigned *sizePerThread,
    const unsigned *threadsPerWarp, const unsigned *warpsPerCTA,
    const unsigned *order, const unsigned *CTAsPerCGA,
    const unsigned *CTASplitNum, const unsigned *CTAOrder) {
  MLIRContext *context = unwrap(ctx);
  ...
  auto ctaLayout = mlir::triton::gpu::CTAEncodingAttr::fromSplitParams(
...
MLIRAttributeRef tritonRustSharedEncodingAttrGet(
    MLIRContextRef ctx, unsigned vec, unsigned perPhase, unsigned maxPhase,
    size_t orderLen, const unsigned *order, int hasLeadingOffset) {
  MLIRContext *context = unwrap(ctx);
  ...
  (void)hasLeadingOffset; // SwizzledSharedEncodingAttr has no leading-offset
                          // param
  auto sharedAttr = mlir::triton::gpu::SwizzledSharedEncodingAttr::get(
...
int tritonRustAttrIsSharedEncoding(MLIRAttributeRef attr) {
  return isa<mlir::triton::gpu::SwizzledSharedEncodingAttr>(*unwrap(attr));
}
...
MLIRAttributeRef tritonRustSliceEncodingAttrGet(MLIRContextRef ctx,
  ...
  auto parentDist =
      mlir::cast<mlir::triton::gpu::DistributedEncodingTrait>(parentAttr);

  auto sliceAttr =
      mlir::triton::gpu::SliceEncodingAttr::get(context, dim, parentDist);
...
MLIRAttributeRef tritonRustNvidiaMmaEncodingAttrGet(
    MLIRContextRef ctx, unsigned versionMajor, unsigned versionMinor,
    size_t warpsPerCTALen, const unsigned *warpsPerCTA, size_t CTAsPerCGALen,
    const unsigned *CTAsPerCGA, size_t CTASplitNumLen,
    const unsigned *CTASplitNum, size_t CTAOrderLen, const unsigned *CTAOrder,
    size_t instrShapeLen, const unsigned *instrShape) {
  MLIRContext *context = unwrap(ctx);

  llvm::SmallVector<unsigned, 2> wpcVec(warpsPerCTA,
                                        warpsPerCTA + warpsPerCTALen);
  llvm::SmallVector<unsigned, 2> ctasPerCGA(CTAsPerCGA,
                                            CTAsPerCGA + CTAsPerCGALen);
  llvm::SmallVector<unsigned, 2> ctaSplitNum(CTASplitNum,
                                             CTASplitNum + CTASplitNumLen);
  llvm::SmallVector<unsigned, 2> ctaOrder(CTAOrder, CTAOrder + CTAOrderLen);
  llvm::SmallVector<unsigned, 2> instrShapeVec(instrShape,
                                               instrShape + instrShapeLen);
...
MLIRAttributeRef tritonRustAMDMfmaEncodingAttrGet(
    MLIRContextRef ctx, unsigned versionMajor, unsigned versionMinor,
    size_t warpsPerCTALen, const unsigned *warpsPerCTA, unsigned mDim,
    unsigned nDim, int isTransposed, size_t CTAsPerCGALen,
    const unsigned *CTAsPerCGA, size_t CTASplitNumLen,
    const unsigned *CTASplitNum, size_t CTAOrderLen, const unsigned *CTAOrder) {
  MLIRContext *context = unwrap(ctx);
  ...
  // AMDMfmaEncodingAttr::get(context, version, warpsPerCTA, instrShape,
  //                         isTransposed, CTALayout, tpw, elementBitWidth)
  llvm::SmallVector<unsigned, 3> instrShapeVec = {mDim, nDim, 0};
  auto mfmaAttr = mlir::triton::gpu::AMDMfmaEncodingAttr::get(
      context, versionMajor, wpcVec, instrShapeVec, isTransposed != 0,
      ctaLayout);
...
MLIRAttributeRef tritonRustCacheModifierAttrGet(MLIRContextRef ctx,
                                                int modifier) {
  MLIRContext *context = unwrap(ctx);
...
MLIRAttributeRef tritonRustEvictionPolicyAttrGet(MLIRContextRef ctx,
                                                 int policy) {
  MLIRContext *context = unwrap(ctx);
...
MLIRAttributeRef tritonRustPaddingOptionAttrGet(MLIRContextRef ctx,
                                                int option) {
  MLIRContext *context = unwrap(ctx);
...
MLIROperationRef tritonRustMakeRangeOp(MLIROpBuilderRef builder,
                                       MLIRLocationRef loc, int32_t start,
                                       int32_t end) {
...
MLIROperationRef tritonRustSplatOp(MLIROpBuilderRef builder,
                                   MLIRLocationRef loc, MLIRValueRef src,
...
MLIROperationRef tritonRustAddPtrOp(MLIROpBuilderRef builder,
                                    MLIRLocationRef loc, MLIRValueRef ptr,
                                    MLIRValueRef offset) {
  OpBuilder *b = unwrap(builder);
  Location location = *unwrap(loc);
...
MLIROperationRef tritonRustLoadOp(MLIROpBuilderRef builder, MLIRLocationRef loc,
                                  MLIRValueRef ptr, MLIRAttributeRef cache,
                                  MLIRAttributeRef evict, int isVolatile) {
  OpBuilder *b = unwrap(builder);
  Location location = *unwrap(loc);
  ...
  if (auto tensorPtrType = dyn_cast<RankedTensorType>(ptrType)) {
    auto ptrElemType =
        cast<mlir::triton::PointerType>(tensorPtrType.getElementType());
...
MLIROperationRef tritonRustStoreOp(MLIROpBuilderRef builder,
                                   MLIRLocationRef loc, MLIRValueRef ptr,
                                   MLIRValueRef value, MLIRAttributeRef cache) {
  OpBuilder *b = unwrap(builder);
  Location location = *unwrap(loc);
...
MLIROperationRef tritonRustDotOp(MLIROpBuilderRef builder, MLIRLocationRef loc,
                                 MLIRValueRef a, MLIRValueRef b_operand,
                                 MLIRValueRef c, int allowTF32,
  ...
  state.addAttribute("maxNumImpreciseAcc",
                     b->getI32IntegerAttr(maxNumImpreciseAcc));

  return wrap(createAndInsertOp(b, state));
}

MLIROperationRef tritonRustReduceOp(MLIROpBuilderRef builder,
                                    MLIRLocationRef loc, MLIRValueRef operand,
                                    int axis) {
  OpBuilder *b = unwrap(builder);
  Location location = *unwrap(loc);
...
MLIROperationRef tritonRustFuncOp(MLIROpBuilderRef builder, MLIRLocationRef loc,
                                  MLIRStringRef name, size_t numInputs,
...
MLIROperationRef tritonRustReturnOp(MLIROpBuilderRef builder,
                                    MLIRLocationRef loc, size_t numValues,
...
MLIROperationRef
tritonRustInsertSliceAsyncOp(MLIROpBuilderRef builder, MLIRLocationRef loc,
                             MLIRValueRef src, MLIRValueRef dst,
                             MLIRValueRef index, MLIRAttributeRef cache,
                             MLIRAttributeRef evict, int isVolatile) {
  OpBuilder *b = unwrap(builder);
  Location location = *unwrap(loc);
