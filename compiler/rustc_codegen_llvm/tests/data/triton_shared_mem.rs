...
/// teenyc-6mv smoke test kernel: no BLOCK_SIZE/dtype generics needed, since
/// `gluon_shared_mem_smoke_test` is a fixed, self-contained op sequence (see
/// its doc comment and `TritonCodegen::codegen_gluon_shared_mem_smoke_test`).
pub fn shared_mem_smoke_test<T: Triton>(out_ptr: T::Pointer<i32>) {
    T::gluon_shared_mem_smoke_test(out_ptr);
}
...
#[no_mangle]
pub extern "C" fn entry_point(out_ptr: *mut i32) {
    let out_ptr = LlvmPointer(out_ptr as *mut _);
    shared_mem_smoke_test::<LlvmTriton>(out_ptr);
}