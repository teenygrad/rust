    ...
    Self::I32Tensor: Mul<i32, Output = Self::I32Tensor>,
{
    ...
    /*------------------------------ teenyc-6mv smoke test ------------------------------*/

    /// Not a general-purpose primitive: a fixed, self-contained smoke test that
    /// stages a constant tile through shared memory (`ttg.local_alloc` ->
    /// `local_store` -> `local_load`) and writes it back through `out_ptr`,
    /// checking whether `Language::GLUON` dispatch is drivable from a real
    /// compiled kernel (see `TritonCodegen::codegen_gluon_shared_mem_smoke_test`).
    fn gluon_shared_mem_smoke_test(out_ptr: Self::Pointer<i32>);

    /// Mixed-ops probe: stages a value that came from a *normal*
    /// (`Language::TRITON`-shaped, unencoded) op — e.g. `T::load` — through
    /// shared memory and returns it, so the surrounding kernel can keep using
    /// ordinary `program_id`/`arange`/`load`/`store`. Tests whether Gluon's
    /// encoding-inference pass can backfill the missing `#ttg.blocked`
    /// encoding on `x` from context, rather than requiring every op in the
    /// kernel to be hand-encoded (see `codegen_gluon_shared_mem_roundtrip`).
    fn gluon_shared_mem_roundtrip<D: ty::Dtype>(x: Self::Tensor<D>) -> Self::Tensor<D>;
}
pub mod llvm {
pub use super::super::*;
/*
...
impl Triton for LlvmTriton {
    ...
    #[inline(never)]
    fn gluon_shared_mem_smoke_test(_out_ptr: Self::Pointer<i32>) {
        // dummy implementation not used in final output
    }

    #[inline(never)]
    #[allow(clippy::zero_ptr)]
    fn gluon_shared_mem_roundtrip<D: ty::Dtype>(_x: Self::Tensor<D>) -> Self::Tensor<D> {
        tensor::LlvmTensor(0 as *mut D)
    }
}
pub mod tensor {
...
/// Mixed-ops probe kernel: ordinary program_id/arange/load/store
/// (Language::TRITON-shaped, unencoded) surrounding one
/// `gluon_shared_mem_roundtrip` stage. Tests whether Gluon's
/// encoding-inference pass tolerates the unencoded `x` value it receives
/// from `T::load`, or whether every op needs to be hand-encoded like
/// `gluon_shared_mem_smoke_test`'s fully self-contained sequence.
pub fn shared_mem_roundtrip_kernel<T: Triton, const BLOCK_SIZE: i32>(
    input_ptr: T::Pointer<i32>,
    output_ptr: T::Pointer<i32>,
    n_elements: i32,
) where
    ...
    T::Pointer<i32>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<i32>>>,
{
    ...
    let y = T::gluon_shared_mem_roundtrip(x);
    T::store(output_ptr.add_offsets(offsets), y, Some(in_bounds), &[], None, None);
}
...
#[no_mangle]
pub extern "C" fn entry_point(input_ptr: *mut i32, output_ptr: *mut i32, n_elements: i32) {
    let input_ptr = LlvmPointer(input_ptr as *mut _);
    let output_ptr = LlvmPointer(output_ptr as *mut _);
    shared_mem_roundtrip_kernel::<LlvmTriton, 128>(input_ptr, output_ptr, n_elements);
}