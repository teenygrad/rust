    ...
    Self::I32Tensor: Mul<i32, Output = Self::I32Tensor>,
{
    ...
    /// CTA-wide handshake between an indexed shared-memory write and a later read.
    fn shared_barrier();

    /// Transpose a rank-2 indexed shared-memory buffer's logical row/column order.
...
pub use triton::*;
pub fn indexed_shared_mem_kernel<T: Triton, D: Dtype>(
...
{
    let offsets = T::arange(0, 128);
    let tile =
        T::load(input_ptr.add_offsets(offsets), None, None, &[], None, None, None, false);

    let buf = T::shared_alloc::<D>(&[128, 128]);
    T::shared_store_index(buf, 0, tile);
    T::shared_barrier();
    let buf_t = T::shared_trans(buf);
    let loaded = T::shared_load_index(buf_t, 0);

    T::store(output_ptr.add_offsets(offsets), loaded, None, &[], None, None);
}
...
#[no_mangle]
pub extern "C" fn entry_point(input_ptr: *mut i32, output_ptr: *mut i32) {
    let input_ptr = LlvmPointer(input_ptr as *mut _);
    let output_ptr = LlvmPointer(output_ptr as *mut _);
    indexed_shared_mem_kernel::<LlvmTriton, i32>(input_ptr, output_ptr);
}