...
/// Device-side tile: a tensor (or pointer-tensor) plus an optional boundary mask.
/// teenyc-3af.1: this is a kernel-local value only — never a kernel-entry ABI type.
struct Tile<P, M> {
    tensor: P,
    mask: Option<M>,
}

pub fn struct_option_kernel<T: Triton, D: Float, const BLOCK_SIZE: i32>(
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    // Some(mask) field: construct, field-project into load, and copy the Option field
    // to a local (`let m = tile.mask`) before passing it — both paths must work.
    let in_tile = Tile { tensor: input_ptr.add_offsets(offsets), mask: Some(in_bounds) };
    let m = in_tile.mask;
    let x = T::load(in_tile.tensor, m, None, &[], None, None, None, false);

    let y = T::zeros_like(x);
    let relu = T::maximum(x, y);

    // None mask field: store without a boundary mask, via the struct field.
    let out_tile = Tile { tensor: output_ptr.add_offsets(offsets), mask: None };
    T::store(out_tile.tensor, relu, out_tile.mask, &[], None, None);
}
...
pub extern "C" fn entry_point(input_ptr: *mut f32, output_ptr: *mut f32, n_elements: i32) {
    ...
    struct_option_kernel::<LlvmTriton, f32, 1024>(input_ptr, output_ptr, n_elements);
}