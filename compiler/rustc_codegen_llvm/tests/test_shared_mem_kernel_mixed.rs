...
//! teenyc-6mv follow-up: can a kernel mix *normal* (`Language::TRITON`-shaped,
//! unencoded) ops — `program_id`/`arange`/`load`/`store` — with a
//! `ttg.local_alloc`/`local_store`/`local_load` shared-memory stage, or does
//! Gluon's encoding-inference pass need every op in the kernel to be
//! hand-encoded like `test_shared_mem_kernel.rs`'s fully self-contained
//! sequence?
//!
//! Unlike that test, this one does NOT rebuild its shared-memory operand
//! with an explicit `#ttg.blocked` encoding — `codegen_gluon_shared_mem_roundtrip`
//! takes `x` exactly as `T::load` produced it.
...
mod tests {
    ...
    #[test]
    fn test_gluon_shared_mem_mixed_kernel() -> Result<(), Box<dyn std::error::Error>> {
        let _ = fmt()
            .with_env_filter(
        ...
        let kernel = env::current_dir()
            .unwrap()
            .join("tests/data/triton_shared_mem_mixed.rs");
        let target = "nvptx64-nvidia-cuda";

        println!("Compiling mixed-ops shared-mem kernel with target: {}", kernel.display());
        let output_path = compiler.compile(&kernel, target, "test_gluon_shared_mem_mixed")?;
